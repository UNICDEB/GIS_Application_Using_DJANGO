...
require([
    ...
    window.addMarker = function() {
        ...
        fetchWeatherData(lat, lng, function(weather) {
            ...
            // Add AQI color layers (three circles with different AQI values)
            addAQILayers(lat, lng, weather.aqi);
    ...
    // Function to add three AQI layers based on the AQI value
    function addAQILayers(lat, lng, initialAQI) {
        const fixedLength = 1;  // Decrease AQI by 1 for each layer

        // Layer 1: Actual AQI (innermost)
        addAQICircle(lat, lng, initialAQI, 0.05);

        // Layer 2: Decrease AQI by fixed length
        addAQICircle(lat, lng, Math.max(initialAQI - fixedLength, 1), 0.08);

        // Layer 3: Further decrease
        addAQICircle(lat, lng, Math.max(initialAQI - 2 * fixedLength, 1), 0.11);
    }

    // Function to add a circular layer to represent AQI coverage
    function addAQICircle(lat, lng, aqi, radius) {
        // Determine color based on AQI value
        var color;
        if (aqi == 1) color = [144, 238, 144, 0.4];  // Light green (Good)
        else if (aqi == 2) color = [135, 206, 235, 0.4];  // Light blue (Fair)
        else if (aqi == 3) color = [255, 165, 0, 0.4];  // Orange (Moderate)
        else if (aqi == 4) color = [160, 32, 240, 0.4];   // Purple (Poor)
        else color = [255, 0, 0, 0.4];   // Red (Very Poor)
